...
public:
	...
	T *m_P;
public:
	OMatrix(const UINT rows = 0, const  UINT cols = 0){
			UINT m, n;
			m_P = 0;
			if (rows<0 || cols<0)
				return;
	...
	OMatrix(T* farr, int rows, int cols){
		m_Rows = rows;
		m_Cols = cols;
		int size = m_Rows*m_Cols;
		m_P = new T[size];
	...
	///////////////////////////////////////////////////////////////////////////////////////////
	OMatrix operator+(OMatrix &rc){
		if (m_Rows != rc.m_Rows || m_Cols != rc.m_Cols)
			return OMatrix();
		OMatrix r(m_Rows, m_Cols);
	...
	///////////////////////////////////////////////////////////////////////////////////////////////
	OMatrix GetRows(const int rows){
		if (rows > m_Rows)
			return OMatrix();
	...
	OMatrix GetCols(const int cols){
		if (cols > m_Cols)
			return OMatrix();
